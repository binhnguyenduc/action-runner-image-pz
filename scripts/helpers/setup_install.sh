...
if [ "$SETUP" == "minimal" ]; then
    ...
    echo "Starting complete setup for $IMAGE_OS $IMAGE_VERSION"
    if [[ "$IMAGE_OS" == *"ubuntu"* ]]; then
        if [[ "$IMAGE_VERSION" == "24.04" ]]; then
            ...
            echo "Invalid IMAGE_VERSION value for Ubuntu complete setup. Please set IMAGE_VERSION to contain '22.04' or '24.04'."
            exit 1
        fi
    elif [[ "$IMAGE_OS" == *"rocky"* ]] || [[ "$IMAGE_OS" == *"centos"* ]]; then
        # List of scripts available for Rocky/CentOS complete setup
        SCRIPT_FILES=(
            "install-actions-cache.sh"
            "install-dotnetcore-sdk.sh"
            "install-runner-package.sh"
        ...
        echo "Invalid IMAGE_OS value for complete setup. Supported: ubuntu (22.04/24.04), rocky, centos."
        exit 1
    fi
else
